    ...
    // ================= FITUR BERBAGI / SEDEKAH USAHA IBU =================
    function openIbuSocialModal(defaultType = "gas") {
      const modal = document.getElementById("ibuSocialModal");
      if (!modal) return;
      const todayStr = window.DateHelper ? window.DateHelper.getTodayWIBString() : new Date().toISOString().split("T")[0];
      const dateEl = document.getElementById("socialDate");
      const recipientEl = document.getElementById("socialRecipient");
      const noteEl = document.getElementById("socialNote");

      if (dateEl) dateEl.value = todayStr;
      if (recipientEl) recipientEl.value = "";
      if (noteEl) noteEl.value = "";

      const radios = document.querySelectorAll('input[name="socialType"]');
      radios.forEach(r => {
    ...
    function closeIbuSocialModal() {
      const modal = document.getElementById("ibuSocialModal");
      if (modal) modal.classList.add("hidden");
    }

    function toggleSocialGiftType() {
      const selected = document.querySelector('input[name="socialType"]:checked');
      const amountEl = document.getElementById("socialAmount");
      const inv = window.IbuGasModule ? window.IbuGasModule.getGasInventory() : { hargaModalDefault: 18500 };
      if (selected && selected.value === "gas") {
        if (amountEl) {
          amountEl.value = inv.hargaModalDefault || 18500;
          amountEl.readOnly = true;
          amountEl.classList.add("bg-slate-50", "text-slate-500");
        }
      } else {
        if (amountEl) {
          amountEl.readOnly = false;
          amountEl.classList.remove("bg-slate-50", "text-slate-500");
          if (!amountEl.value || Number(amountEl.value) === 18500) {
    ...
    function submitIbuSocialGift() {
      const selected = document.querySelector('input[name="socialType"]:checked');
      const type = selected ? selected.value : "gas";
      const recipient = (document.getElementById("socialRecipient") ? document.getElementById("socialRecipient").value.trim() : "") || "Mbah / Tetangga";
      const dateVal = document.getElementById("socialDate") ? document.getElementById("socialDate").value : "";
      const amount = Number(document.getElementById("socialAmount") ? document.getElementById("socialAmount").value : 0);
      const note = document.getElementById("socialNote") ? document.getElementById("socialNote").value.trim() : "";

      if (type === "cash" && (!amount || amount <= 0)) {
        showToast("Nominal santunan tunai harus lebih dari 0!", "warning");
        return;
      }

      if (window.IbuGasModule && window.IbuGasModule.recordIbuSocialGift) {
        const success = window.IbuGasModule.recordIbuSocialGift({
          type,
          recipient,
          amount,
          note,
        ...
        if (success) {
          ...
          showToast(`Alhamdulillah, catatan berbagi untuk ${recipient} berhasil dicatat! 🎁`, "success");
        }
      }
    }
    ...
    function renderIbuTransactionList() {
      ...
      } else if (filterUnit === "social") {
        txs = txs.filter(t => (t.category && (t.category.includes("Sedekah") || t.category.includes("Berbagi"))) || (t.note && (t.note.includes("Sedekah") || t.note.includes("Gratis") || t.note.includes("Bantuan") || t.note.includes("Mbah"))));
      }
      ...
      if (txs.length === 0) {
        container.innerHTML = `
          <div class="text-center py-8 text-slate-400 text-xs font-bold space-y-1">
            ...
            <div class="text-[10px] text-slate-400 font-normal">Riwayat pembayaran sewa kost, gas LPG, dan sedekah berbagi akan tercatat di sini.</div>
          </div>
        `;
        return;
      }
      ...
      container.innerHTML = txs.map(t => {
        const isSocial = (t.category && (t.category.includes("Sedekah") || t.category.includes("Berbagi"))) || (t.note && (t.note.includes("Sedekah") || t.note.includes("Gratis") || t.note.includes("Bantuan") || t.note.includes("Mbah")));
        ...
        if (isSocial) {
          badgeClass = "bg-teal-100 text-teal-900 border border-teal-200/80 font-black";
          badgeLabel = "🎁 Berbagi";
        } else if (isKost) {
          ...
          <div class="p-2.5 rounded-xl border border-slate-200/80 bg-white flex items-center justify-between gap-2 shadow-2xs hover:border-slate-300 transition-all">
            <div class="space-y-0.5 min-w-0 flex-1">
              <div class="flex items-center gap-1.5 flex-wrap">
                <span class="text-[9.5px] px-2 py-0.5 rounded-md ${badgeClass}">
                  ${badgeLabel}
                </span>
              ...
              <div class="text-xs font-black text-slate-900 truncate" title="${t.note || '-'}">
                ${t.note || (isKost ? 'Penerimaan sewa kost' : (isSocial ? 'Sedekah / Berbagi' : 'Transaksi gas'))}
              </div>
            </div>

            <div class="flex items-center gap-2 shrink-0">
              <div class="text-right">
                <div class="text-xs font-black ${isIncome ? 'text-emerald-600' : (isSocial ? 'text-teal-700' : 'text-rose-600')}">
